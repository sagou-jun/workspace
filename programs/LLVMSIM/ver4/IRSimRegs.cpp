...
inline string IRSIM::str_Kind(KINDS k){
  switch(k){
  case INTEGER:
    return "INTEGER"; 
  case STRING:
...
RegTableELE::RegTableELE(unsigned id,string name,KINDS k){
  RegID = id;
  Name = name;
  Kind = k;
  trace = false;
...
RegTableELE *RegTable::setRegInt(int value,string name){
  RegTableELE *q;
...
RegTableELE *RegTable::setRegDouble(double value,string name){
  RegTableELE *q;
  if((q = RegLookUp(name)) == NULL)
    {
...
RegTableELE *RegTable::setRegAdd(unsigned value,string name){
  RegTableELE *q;
  if((q = RegLookUp(name)) == NULL)
    {
