...
int main(int argc,char *argv[],const *envp){
  ...
  string FileName("C:\\cygwin64\\usr\\local\\programs\\clang_build\\PID_controller_PILS.ll");
  ...
  cerr<<"RunPID_controller_PILS::Arguments check"<<endl;
  if(argc < 3)
    {
      cerr<<"RunPID_controller_PILS::Invalid arguments"<<endl;
      exit(1);
    }
  ...
  while(Terminate)
      ...
      case XIL_INITIALIZE_COMMAND:
	{
	  ...
	  fflush(fp);
	  //string FunctionName("PID_controller_PILS_initialize");
	  SIM.RunThisFunction(rtn,"PID_controller_PILS_initialize");
	  /*data export*/
      ...
      case XIL_DISABLE_COMMAND:
	{
	  ...
	  if(command == XIL_STEP_COMMAND)
		...
		for(int j = 0;((j < sizeof(real_T))&&(size > 0));j++)
	      ...
	      /*XIL_CALL_INTERFACE*/
	      //string FunctionName("PID_controller_PILS_step");
	      SIM.RunThisFunction(rtn,"PID_controller_PILS_step");
	      fflush(fp);
	      /*XIL_PROCESS_OUTPUTS*/
	      for(int i=0;i < OutDataNum;i++)
		{
