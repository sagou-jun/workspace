...
int main(int argc,char *argv[]){
  ...
  if((relay_recv = fopen("relay2_recv_out.txt","w")) == NULL)
    {
	  ...
	  for(uint32_T i = 0;i < real_T_Size;i++)
	    {
	      ...
	      in[2].field[i] = xilCommsBuffer[(3*real_T_Size+1)+i];
	      in[3].field[i] = xilCommsBuffer[(4*real_T_Size+1)+i];
	    }
	  fprintf(stderr,"in1 = %lf, in2 = %lf, in3 = %lf, in4 = %lf\n",in[0].dataValue,in[1].dataValue,in[2].dataValue,in[3].dataValue);
	}
      fprintf(relay_recv,"\n\n");
      fflush(relay_recv);
	  ...
	  for(int i = 0;i < real_T_Size;i++)
	    {
	      ...
	      out[1].field[i] = xilCommsBuffer[2+real_T_Size+i];
	      out[2].field[i] = xilCommsBuffer[2+2*real_T_Size+i];
	    }
	}
      fprintf(stderr,"out1 = %lf, out2 = %lf, out3 = %lf\n",out[0].dataValue,out[1].dataValue,out[2].dataValue);
      fprintf(relay_recv,"\n\n");
      fflush(relay_recv);
